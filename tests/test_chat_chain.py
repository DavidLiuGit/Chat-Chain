...
class TestBuildQuestionAndAnswerChain(unittest.TestCase):
    ...
    def test_build_qa_chain_no_retriever(self):
        ...
        props = ChatChainProps(
            chat_llm=self.mock_llm,
            chat_prompt=self.chat_prompt_str,
            retriever=None
        )
        ...
        self.assertEqual(
            ...
            sorted(["chat_history", "user_input"])
        )
        
        # Verify the prompt template uses the correct template string
        self.assertEqual(
            chat_chain.qa_prompt_template.template,
            QA_CHAIN_LLM_PROMPT_TEPLATE
        )
        
        # Verify the chain input has the correct keys
        self.assertEqual(
            sorted(chat_chain.chain_input.keys()),
            sorted(["chat_history", "user_input"])
        )
    ...
    def test_build_qa_chain_with_string_prompt(self):
        ...
        self.assertEqual(
            chat_chain.qa_prompt_template.partial_variables["prompt"],
            self.chat_prompt_str
        )
    ...
    def test_build_qa_chain_with_callable_prompt(self):
        ...
        props = ChatChainProps(
            chat_llm=self.mock_llm,
            chat_prompt=self.chat_prompt_callable,
            retriever=None
        )
